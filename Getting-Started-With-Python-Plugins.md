...
So if your @Extension class or its descriptor class inherits from wrapper class instead of original class, it can use ability to implement methods in the associated Python script. For example if you are implementing some `Recorder` extension and it is called `MyRecorder` and this class inherits from `RecorderPW`, there should be attached Python script with name my_recorder.py which may or not contain implementations of `Recorder` methods.
