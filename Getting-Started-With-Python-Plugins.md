...
<p align="center">
<img src="http://raw.githubusercontent.com/jenkinsci/jenkins.py/master/concept.png" width="100%" height="100%" />
</p>
...
*  _Types of the terminal plugin are not visible in Python scripts._  
   The reason is that Python scripts are executed by the parent plugin, python-wrapper, which can not see types of the terminal plugin. This will be probably resolved in the future. If you need to create instances of the extension class in the Python script, you can workaround that by calling standard `type()` function on an `extension` object and its descriptor in the `init_plugin()` function:  
